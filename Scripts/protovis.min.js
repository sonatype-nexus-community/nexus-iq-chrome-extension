/**
 * @preserve Copyright (c) 2010, Stanford Visualization Group
 * @license http://mbostock.github.com/protovis/license.txt
 */
...
pv.SvgScene.rule=function(b){for(var c=b.$g.firstChild,d=0;d<b.length;d++){var f=b[d];if(f.visible){var g=f.strokeStyle;if(g.opacity){c=this.expect(c,"line",{"shape-rendering":f.antialias?null:"crispEdges","pointer-events":f.events,cursor:f.cursor,x1:f.left,y1:f.top,x2:f.left+f.width,y2:f.top+f.height,stroke:g.color,"stroke-opacity":g.opacity,"stroke-dasharray":f.strokeDasharray,"stroke-width":f.lineWidth/this.scale});c=this.append(c,b,d)}}}return c};
...
pv.Rule.prototype=pv.extend(pv.Mark).property("width",Number).property("height",Number).property("lineWidth",Number).property("strokeDasharray",String).property("strokeStyle",pv.color);pv.Rule.prototype.type="rule";pv.Rule.prototype.defaults=(new pv.Rule).extend(pv.Mark.prototype.defaults).lineWidth(1).strokeStyle("black").strokeDasharray("").antialias(false);pv.Rule.prototype.anchor=pv.Line.prototype.anchor;
