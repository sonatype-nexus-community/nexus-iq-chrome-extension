...
// (function() {
  ...
  function artifactsChart(known, config) {
    console.log("artifactsChart", known, config);
  ...
  ComponentInformation = (function() {
    ...
    function loadVersionChart(config) {
      ...
      if (config.panning) {
        ...
        vizContent = panWrapper.add(pv.Panel).overflow('hidden').height(config.actualHeight).width(config.contentWidth -
            30).fillStyle(bgBlue).strokeStyle(bgBlue).left(15);
      }
      ...
      for (i = ((config.width / 2) - 1) % config.spacer - config.spacer / 2 - 0.5; i < config.width;
           i += config.spacer) {
